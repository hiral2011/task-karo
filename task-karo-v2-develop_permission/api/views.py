...
# def mobile_validater(number_str):
#     validator_string = list("+1234567890")
#     invalid = 0
#     for ch in number_str:
#         if ch not in validator_string:
#             invalid = 1
#     return invalid
...
#         if password=="":
#             return Response({'status' : 406 ,'message' : "password is required"},status=status.HTTP_406_NOT_ACCEPTABLE)
    
#         if validate_name(first_name):
#             return Response({'status' : 406 ,'message' :"special charecter not acceptable"},status=status.HTTP_406_NOT_ACCEPTABLE)
...
#         token = create_token(user=new_user)
#         new_user.save()
#         family=Family.objects.create(family_head=new_user,relation="self")
#         # if new_user.family == family:
#         permission = Permissions.objects.create(permission=self.admin_permission,role_id=1,user_id=new_user,family_id=family)
#         # else :
#         #     permission = Permissions.objects.create(permission=self.admin_permission,role_id=2,user_id=new_user,family_id=family)
#         # print(family.user_id.add(new_user))
#         data=serializer.data
#         data['token'] = token
#         return Response({'data' : data ,
#                         "status_code": 201,
#                         "response_error": False,
#                         'status_message': "User data added successfully!"},status=status.HTTP_201_CREATED)         
    ...
    def list(self,request):
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            if user:
                ...
                return Response({"status":1,"message":"User profile.","data":serializer.data})
            else:
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            if user:
                ...
                if serializer.is_valid():
                    ...
                    #     'country_code': country_code,
                    # }
                    return Response({"status":1,"message":"User profile Updated OTP has been sent successfully on your mobile number.",'data':serializer.data})
            else:
...
class LogInViewSet(viewsets.ModelViewSet):
    ...
    def create(self,request):
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            return Response({"status":0,"message":"Invalid API Key."})
    ...
    def create(self,request):
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            try:
                ...
                family = Family.objects.get(Q(user_id__in=[user]))
            ...
            if user:
                try:
                    ...
                    if not serializer.is_valid():
                        print("hii",serializer.errors)
                        return Response({'data':{},'status_message':{
                            "Expose": True,
                            "Status": 0,
                            "Massage":serializer.errors,
                            "Type": "entity.parse.failed"},
                            "Response_error": True},status=status.HTTP_406_NOT_ACCEPTABLE)
                    else: 
                        new_loc = serializer.save(family_id=family)
    ...
    def update(self,request,pk): 
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            if user:
                ...
                print("_______________________________________________",location)
                family = Family.objects.get(Q(user_id__in=[user])|Q(family_head=user))
                print("_______________________________________________",family)
                serializer = Location_Serializer(instance=location,data=request.data,partial=True)
                if serializer.is_valid():
                    serializer.save(family_id=family)
                    return Response({'Data':serializer.data ,'Status': 1 ,'Message' : 'Location Data Updated Successfully'},status=status.HTTP_200_OK)
                return Response({'data':{},'status': 400 , 'errors' : serializer.errors},status=status.HTTP_400_BAD_REQUEST)
            else:
                return Response({"Status":0,"Message":"User does not exist"},status = status.HTTP_401_UNAUTHORIZED)  
        else:
            return Response({"status":0,"Message":"Invalid API Key."},status = status.HTTP_401_UNAUTHORIZED)
            ...
            if user:
                try:
                    ...
                    if serializer.data:
                        return Response({'data': serializer.data,
                                    "status_code": 1,
                                    "status_message": "City Records Fetched Successfully",
                                    "response_error":False},status=status.HTTP_201_CREATED)
                    else:
            ...
            if user:
                try:
                    ...
                    if not serializer.is_valid():
                        ...
                        new_Service_provider = serializer.save(family_id=family,location_id=location_id)
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            if user:
                ...
                try:
                    ...
                    service_provider = Service_provider.objects.filter(Q(family_id__user_id__in=[user])|Q(family_id__family_head=user))
...
#add Appliance by post
    def create(self,request):
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            # location = Location.objects.get(family_id=family.family_id)
            # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@",location)
            ...
            if user:
                ...
                if not serializer.is_valid():
                    ...
                    new_appliance = serializer.save(family_id=family,location_id=location_id,service_provider_id=service_provider)
...
# all appliance by get
    def list(self,request):
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            if user:
                try:
                    appliance = Appliances.objects.filter(family_id__family_head=user)
                    family=Family.objects.get(family_head=user)
                    print("****************************",appliance)
                    #appliance = Appliances.objects.all()
                    if not appliance:
    ...
    def create(self,request):
        ...
        if user:
            ...
            if not serializer.is_valid():
                ...
                task = serializer.save(family_id=family,service_provider_id=service_provider)
...
# All Task Listing
    def list(self,request):
        ...
        if apikey.strip() == API_KEY.strip():
            ...
            if user:
                try:
                    task = Task.objects.filter(family_id__family_head=user)
                    family=Family.objects.get(Q(family_id__user_id__in=[user])|Q(family_id__family_head=user))
                    print("****************************",task)
                    if not task:
                        return Response({"Status":0,"Message":"No data Found"},status=status.HTTP_404_NOT_FOUND)
...
class Subscription_PlanAPI(viewsets.ModelViewSet):
    ...
    def list(self,request):
        try:
            ...
            return Response({'Data': serializer.data,
                            "Status": 1,
                            "Message":"Subscription_Plan data fetched successfully",
                            "Response_error": False
                            },status=status.HTTP_200_OK)
