...
class ProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("user_id","first_name","last_name","mobile_number","country_code","email","password",'gender')
...
class ProfileSocialUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("user_id","first_name","last_name","mobile_number","country_code","password","is_social_login","social_id","social_type","profile_image",'gender','is_mobile_verified','is_email_verified')
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("user_id","email","first_name","last_name","device_info","profile_image","is_social_login","is_profile_completed","social_id","social_type","profile_image","mobile_number","country_code",'gender','is_mobile_verified','is_email_verified')
...
class ProfileTokenSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("user_id","email","first_name","last_name","token","device_info","profile_image","is_social_login","is_profile_completed","social_id","social_type","profile_image","mobile_number","country_code",'gender','is_mobile_verified','is_email_verified')
...
class Location_Serializer(serializers.ModelSerializer):
   class Meta:
        model = Location
        fields =  ['location_id','location_name','family_id','address','state_id','city_id','latitude','longitude','active','is_deleted']
...
class Appliances_Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Appliances
        #fields = '__all__'
        fields =  ['appliance_id','service_provider_id','family_id','location_id','appliance_type','bought_from','appliance_name','mobile_number','alternativ_mobile_number','price','invoice_number','invoice_date','billing_email','billing_name','billing_mobile_number','warranty_last_date','product_image','is_amc_service_available','company_name','contact_person_name','amc_phone_number','support_email','amc_service_address','service_time_period','service_provider_id','appliance_note','upload_invoice_pdf_img','active','is_deleted','created_at','updated_at','deleted_at','created_by','updated_by','deleted_by']
        depth = 1
